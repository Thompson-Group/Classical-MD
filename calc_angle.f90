...
!cfac is a common parameter for all the forces
                cfac = -k_ang(d)*(rad_ang - rad_equi)/sqrt(1 - catmp **2)
		v_a = v_a + 0.5*k_ang(d)*(rad_ang-rad_equi)**2 
                fx_a(a) = fx_a(a) - cfac*(rx(b,c)/(lenR1*lenR2) - catmp*rx(a,b)/lenR1**2)
                fy_a(a) = fy_a(a) - cfac*(ry(b,c)/(lenR1*lenR2) - catmp*ry(a,b)/lenR1**2)
                fz_a(a) = fz_a(a) - cfac*(rz(b,c)/(lenR1*lenR2) - catmp*rz(a,b)/lenR1**2)

		fx_a(b) = fx_a(b) + cfac*(rx(a,b)+rx(b,c))/(lenR1*lenR2) - catmp*(rx(a,b)/lenR1**2 + rx(b,c)/lenR2**2)
		fy_a(b) = fy_a(b) + cfac*(ry(a,b)+ry(b,c))/(lenR1*lenR2) - catmp*(ry(a,b)/lenR1**2 + ry(b,c)/lenR2**2)
		fz_a(b) = fz_a(b) + cfac*(rz(a,b)+rz(b,c))/(lenR1*lenR2) - catmp*(rz(a,b)/lenR1**2 + rz(b,c)/lenR2**2)
               
		fx_a(c) = fx_a(c) - cfac*(rx(a,b)/(lenR1*lenR2) - catmp*rx(b,c)/lenR2**2)
		fy_a(c) = fy_a(c) - cfac*(ry(a,b)/(lenR1*lenR2) - catmp*ry(b,c)/lenR2**2)
		fz_a(c) = fz_a(c) - cfac*(rz(a,b)/(lenR1*lenR2) - catmp*rz(b,c)/lenR2**2)
