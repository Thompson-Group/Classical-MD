...
contains
	subroutine calc_angle(rx,ry,rz,natms,nangles,k,theta0,At1,at2, at3,bond_energy, Fx, Fy, Fz)
	use kinds
	implicit none

	integer(kind=ip),intent(in) :: natms, nangles
	real(kind=dp),intent(in)    :: rx(natms,natms),ry(natms,natms),rz(natms,natms)
        real(kind=dp),intent(in)    :: k,theta0
        real(kind=dp),intent(out)    ::bond_energy
        integer(kind=ip), intent(in) :: at1(natms), at2(natms), at3(natms)
	real(kind=dp),intent(out)   :: fx(natms),fy(natms),fz(natms)
	integer(kind=ip) :: i,a,b,c
	...
	do i=1,nangles
		...
		rad_ang = acos(dot_prod/(lenR1*lenR2))
                ...
                cfac = -k*(rad_ang - theta0)/sqrt(1 - catmp **2)
		bond_energy = bond_energy + 0.5*k*(rad_ang-theta0)**2 
