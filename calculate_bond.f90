subroutine calculate_bond(n_atom, n_bond, n_b_type, k, req, x, y, z, rx, ry, rz, fx_b, fy_b, fz_b, bond_table, bond_pe, bond_style)
    use kinds
    implicit none
    ...
    real(kind=dp), dimension(n_bond):: rinst
    !2D Arrays
    real(kind=dp), dimension(n_atom,n_atom) :: rx, ry, rz
    !Arrays for storing information, all of n_atom length.
    real(kind=dp), dimension(n_atom) :: x,y,z,fx_b, fy_b, fz_b
    ...
    IF (bond_style.eq."harmonic") THEN
        ...
        DO i = 1, n_bond
            ...
            fx_b(b_atom_1) = - k(b_type) * r_sep * rx(b_atom_1,b_atom_2) / rinst(i)
            fx_b(b_atom_2) = - fx_b(b_atom_1)
            fy_b(b_atom_1) = - k(b_type) * r_sep * ry(b_atom_1,b_atom_2) / rinst(i)
            fy_b(b_atom_2) = - fy_b(b_atom_1)
            fz_b(b_atom_1) = - k(b_type) * r_sep * rz(b_atom_1,b_atom_2) / rinst(i)
